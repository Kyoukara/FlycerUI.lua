...
ShowNotification({
	...
	onComplete = function()
		...
		-- Separator below tab rail (absolute offset from top of MainFrame)
		makeSeparator(MainFrame, TAB_RAIL_Y + TAB_HEIGHT + 6, false)

		-- ═══════════════════════════════════════════════════════════
		-- CONTENT AREA — ShadowFrame Build
		-- ═══════════════════════════════════════════════════════════

		local ShadowFrame = Instance.new("Frame")
		ShadowFrame.Name = "ShadowFrame"
		ShadowFrame.Size = initLayout.shadowSize
		ShadowFrame.Position = initLayout.shadowPos -- ambil dari calcLayout
		...
		-- ExtraFrame tracks MainFrame position automatically
		...
		-- ═══════════════════════════════════════════════════
		-- TAMBAHKAN FUNGSI setUILock DI SINI
		-- ═══════════════════════════════════════════════════
		...
		-- Accent bar tweens — created once, reused on every toggle
		...
		--[[
local PC_KEYBIND_ENABLED = false

if PC_KEYBIND_ENABLED then
    table.insert(eventConnections,
        UserInputService.InputBegan:Connect(function(input, gameProcessed)
            if gameProcessed then return end
            if not isPC then return end

            if input.KeyCode == PC_TOGGLE_KEY_PRIMARY
            or input.KeyCode == PC_TOGGLE_KEY_ALT then
                toggleGUI()
            end
        end)
    )
		...
		local function makeComponents(canvas)
			...
			setmetatable(Components.Section, {
				__call = function(_, cfg, layoutOrder)
					return Components.Section.New(cfg, layoutOrder)
				end,
			})
			...
			setmetatable(Components.Button, {
				__call = function(_, cfg)
					return Components.Button.New(cfg)
				end,
			})
			...
			setmetatable(Components.Toggle, {
				__call = function(_, cfg)
					return Components.Toggle.New(cfg)
				end,
			})
    ...
    -- Munculkan notifikasi loading (setelah UI selesai dibuat)
    ShowNotification({
        title = GUI_TITLE,
        body = "Script loaded successfully.",
        duration = 3,
