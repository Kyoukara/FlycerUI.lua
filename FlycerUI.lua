-- ═══════════════════════════════════════════════════════════════════════════
-- FLYCER UI V2.0.0 - FIXED FADE SYSTEM
-- ═══════════════════════════════════════════════════════════════════════════
...
local function processNotifQueue()
	...
	task.spawn(function()
		while #_notifQueue > 0 do
			...
			TweenService
				:Create(progressFill, TweenInfo.new(duration, Enum.EasingStyle.Linear), { Size = UDim2.new(0, 0, 1, 0) })
				:Play()
			task.wait(duration)
...
local function openResizeGUI(refs)
	...
	changeBtnGrad.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0.0, Color3.fromRGB(40, 180, 100)),
		ColorSequenceKeypoint.new(0.5, Color3.fromRGB(50, 200, 110)),
		ColorSequenceKeypoint.new(1.0, Color3.fromRGB(60, 210, 120)),
	})
	...
	local function applyResize()
		...
		TweenService
			:Create(changeBtn, TweenInfo.new(0.1), { BackgroundColor3 = RX.Green, BackgroundTransparency = 0 })
			:Play()
		...
		task.delay(0.25, function()
			closeResizeGui(function()
				...
				if SF and SF.Parent then
					TweenService
						:Create(SF, tweenInfo, { Size = newLayout.shadowSize, Position = newLayout.shadowPos })
						:Play()
				end
				if TabSF and TabSF.Parent then
					TweenService
						:Create(TabSF, tweenInfo, { Size = newLayout.tabShadowSize, Position = newLayout.tabShadowPos })
						:Play()
				end
...
function FlycerUI:CreateWindow(config)
	...
	if isMobile then
		...
		TabScroll.ScrollBarImageTransparency = 0.7
	...
	UserInputService.InputEnded:Connect(function(input)
		...
		then
			if isDragging then
				isDragging = false
				TweenService
					:Create(DragBar, TWEEN_FAST, { BackgroundTransparency = isHovered and 0.50 or 0.80 })
					:Play()
			end
		end
	end)
	...
	-- ════════════════════════════════════════════════════════════════════════════════
	-- FIXED FADE SYSTEM - SYNCHRONIZED
	-- ════════════════════════════════════════════════════════════════════════════════
	...
	-- Cache transparency untuk SEMUA objek tanpa exception
	local function cacheTransparency(obj)
		if not obj or not obj.Parent then
			return
		end

		local className = obj.ClassName
		if EXCLUDED_CLASSES[className] then
			return
		end
	...
	-- Cache semua descendants
	local function cacheAllTransparencies(root)
		originalTransparencies = {}

		-- Cache root
		cacheTransparency(root)

		-- Cache semua descendants
		for _, descendant in ipairs(root:GetDescendants()) do
			cacheTransparency(descendant)
		end
	end

	-- Cancel semua tween yang sedang berjalan
	local function cancelAllFadeTweens()
		for _, tween in ipairs(currentFadeTweens) do
			pcall(function()
				tween:Cancel()
			end)
		end
	...
	-- Set transparency instant tanpa tween
	local function setTransparencyInstant(targetAlpha)
	...
	-- Fade semua objek secara SINKRON
	local function fadeAllObjectsSynchronized(targetAlpha, duration, callback)
		cancelAllFadeTweens()
		...
		for obj, props in pairs(originalTransparencies) do
			if obj and obj.Parent then
				...
				for propName, originalValue in pairs(props) do
					if targetAlpha == 0 then
						tweenProps[propName] = originalValue
					else
						tweenProps[propName] = 1
					end
				end
		...
		if tweenCount == 0 then
			setTransparencyInstant(targetAlpha)
			if callback then
				task.spawn(callback)
			end
			return
		...
		-- ✅ KUNCI: Play SEMUA tween dalam 1 frame
		task.spawn(function()
			...
			-- Play semua tween di frame yang sama
			for _, tween in ipairs(tweenList) do
				tween:Play()
			end
		end)
	...
	if not isMobile then
		table.insert(
			...
			ToggleFrame.MouseEnter:Connect(function()
				TweenService
					:Create(ToggleFrame, TWEEN_FAST, { BackgroundTransparency = math.max(RX.MainAlpha - 0.05, 0) })
					:Play()
	...
	local function toggleGUI()
		...
		if mainVisible then
			...
			fadeAllObjectsSynchronized(0, FADE_DURATION, function()
				isAnimating = false
			end)
		else
			-- HIDE UI - Cache semua object
			cacheAllTransparencies(MainFrame)
			if ExtraFrame and ExtraFrame.Parent then
				cacheAllTransparencies(ExtraFrame)
			end
			...
			fadeAllObjectsSynchronized(1, FADE_DURATION, function()
	...
	local function cleanupFadeSystem()
		cancelAllFadeTweens()
	...
	table.insert(eventConnections, ToggleSG.Destroying:Connect(cleanupFadeSystem))
	table.insert(eventConnections, MainGui.Destroying:Connect(cleanupFadeSystem))
	...
	local function resizeHandler()
		...
		TweenService
			:Create(
				...
				{ BackgroundColor3 = Color3.fromRGB(180, 130, 255), BackgroundTransparency = 0.1 }
			)
			:Play()
		TweenService
			:Create(
				resizeStroke,
				TweenInfo.new(0.12, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
				{ Color = Color3.fromRGB(200, 160, 255), Transparency = 0 }
			)
			:Play()
		...
		task.delay(0.22, function()
			...
			TweenService
				:Create(
					...
					{ BackgroundColor3 = RX.Accent2, BackgroundTransparency = 0.8 }
				)
				:Play()
			TweenService
				:Create(
					resizeStroke,
					TweenInfo.new(0.20, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
					{ Color = RX.Accent2, Transparency = 0.3 }
				)
				:Play()
		...
		task.delay(0.22, function()
			...
			TweenService
				:Create(
					discordButton,
					TweenInfo.new(0.20, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
					{ BackgroundColor3 = RX.Accent1, BackgroundTransparency = 0.8 }
				)
				:Play()
			TweenService
				:Create(
					discordStroke,
					TweenInfo.new(0.20, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
					{ Color = RX.Accent1, Transparency = 0.3 }
				)
				:Play()
		...
		for _, tab in ipairs(Window.Tabs) do
			...
			TweenService
				:Create(
					tab.Button,
					TWEEN_FAST,
					{ BackgroundColor3 = active and ACTIVE_BG or INACTIVE_BG, BackgroundTransparency = active and 0 or 0.3 }
				)
				:Play()
			TweenService:Create(tab.Label, TWEEN_FAST, { TextColor3 = active and RX.T1 or RX.T3 }):Play()
			TweenService
				:Create(
					tab.Stroke,
					TWEEN_FAST,
					{ Transparency = active and 0.3 or 0.8, Color = active and RX.Accent1 or RX.Border }
				)
				:Play()
	...
	function Window:Tab(config)
		...
		function Tab:Button(config)
			...
			local function updateLockedVisuals(instant)
				...
				TweenService
					:Create(btn, ti, { BackgroundTransparency = locked and 0.5 or 0.15, TextTransparency = locked and 0.35 or 0 })
					:Play()
			end
		...
		function Tab:Toggle(config)
			...
			local function updateToggle(enabled, instant)
				local ti = instant and TweenInfo.new(0) or TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
				TweenService
					:Create(
						knob,
						ti,
					...
					:Play()
				TweenService:Create(knobGlow, TWEEN_FAST, { BackgroundTransparency = enabled and 0.4 or 1 }):Play()
				TweenService
					:Create(track, TWEEN_NORMAL, { BackgroundColor3 = enabled and RX.Accent1 or Color3.fromRGB(38, 38, 55) })
					:Play()
				TweenService:Create(trackStroke, TWEEN_NORMAL, { Color = enabled and RX.Accent1 or RX.Border }):Play()
			end
		...
		-- ═══════════════════════════════════════════════════════════════════════════
		-- SLIDER, DROPDOWN, TEXTINPUT, COLORPICKER, KEYBIND
		-- (Sisanya sama seperti original, tidak ada perubahan di elemen ini)
		-- ═══════════════════════════════════════════════════════════════════════════

		-- Untuk menghemat space, saya skip sisanya karena tidak ada perubahan
		-- Copy dari script original untuk: Slider, Dropdown, TextInput, ColorPicker, Keybind
